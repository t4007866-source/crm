...
export async function POST(req: NextRequest) {
  ...
  const call = await prisma.serviceCall.create({ data: { callNumber: `SR-${1001 + count}`, customerId: body.customerId, installedSystemId: body.installedSystemId || null, type: body.type, status: body.scheduledAt ? "SCHEDULED" : "OPEN", fault: body.fault || null, description: body.description || body.fault || null, treatmentNotes: body.treatmentNotes || body.notes || null, notes: body.notes || null, cost: body.cost === undefined || body.cost === "" ? null : Number(body.cost), technicianId: body.technicianId || null, scheduledAt: body.scheduledAt ? new Date(body.scheduledAt) : null } });
  if (body.scheduledAt) await prisma.appointment.create({ data: { customerId: body.customerId, installedSystemId: body.installedSystemId || null, technicianId: body.technicianId || null, type: body.type === "FILTER_CHANGE" ? "FILTER_REPLACEMENT" : "TECHNICIAN_SERVICE", title: `${call.callNumber} — ${body.fault || body.type}`, startAtUtc: new Date(body.scheduledAt), endAtUtc: body.endAt ? new Date(body.endAt) : null, timezone: "Asia/Jerusalem", status: "SCHEDULED", notes: body.notes || null } });
