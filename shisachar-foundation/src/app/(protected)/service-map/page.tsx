...
export default function ServiceMapPage() {
  ...
  useEffect(() => {
    ...
    const script = document.createElement("script"); script.id = "google-maps-script"; script.async = true; script.defer = true; script.src = `https://maps.googleapis.com/maps/api/js?key=${encodeURIComponent(key)}`; script.onload = () => setReady(true); script.onerror = () => setError("טעינת Google Maps נכשלה. בדוק את המפתח וההרשאות ב-Google Cloud."); document.head.appendChild(script);
  }, []);
  ...
  useEffect(() => {
    ...
    const visible = customers.filter((c) => (!statusFilter || c.status === statusFilter) && (!query || `${c.name} ${c.company || ""} ${c.city || ""}`.toLowerCase().includes(query.toLowerCase())));
    visible.forEach((c) => {
